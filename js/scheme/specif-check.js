...
//				console.log( rt );
    ...
    function checkConstraints(data) {
        ...
        function checkUniqueIds(iE) {
            ...
            if (dId) return {
                status: 912,
                statusText: "objectType or attributeType identifier '" + dId + "' is not unique"
            };
            dId = duplicateId(iE.relationTypes);
            if (dId) return {
                status: 913,
                statusText: "relationType or attributeType identifier '" + dId + "' is not unique"
            };
            dId = duplicateId(iE.hierarchyTypes);
            if (dId) return {
                status: 914,
                statusText: "hierarchyType or attributeType identifier '" + dId + "' is not unique"
            };
        ...
        function checkNodes(L, nds) {
            ...
            for (var i = 0; i < nds.length; i++) {
                if (indexById(L, nds[i].object) < 0) return {
                    status: 909,
                    statusText: "hierarchy node with identifier '" + nds[i].id + "' must reference a valid object"
        ...
        function checkDataTypes(L) {
            for (var i = 0, I = L.length; i < I; i++) {
                switch (L[i].type) {
                    ...
                    // no break;
                    case 'xs:integer':
                        if (L[i].min == undefined || L[i].max == undefined || L[i].min + 1 > L[i].max) return {
                            status: 929,
                            statusText: "number types must have min and max"
                        };
                        break;
                }
            }
        ...
        function indexById(L, id) {
            if (!L || !id) return -1;
            // given an ID of an element in a list, return its index:
            id = id.trim();
            for (let i = 0; i < L.length; i++) {
                if (L[i].id.trim() === id) return i;
            }
            return -1;
        }
        ...
        function checkTypes(L, els, type) {
            ...
            for (var i = 0; i < els.length; i++) {
                ...
                if (sTi < 0) return {
                    status: 903,
                    statusText: "instance with identifier '" + els[i].id + "' must reference a valid type"
                }
//			if( !checkAttrTypeIds(L[sTi].attributeTypes,els[i].attributes) ) return {status:920, statusText: "attributes of instance with identifier '"+els[i].id+"' must reference valid attributeTypes"}
        ...
        function checkAttrTypes(L, sTs) {
            ...
            for (var i = 0; i < sTs.length; i++) {
                if(!sTs[i].attributeTypes)continue;
                for (var j = 0; j <  sTs[i].attributeTypes.length; j++) {
                    ...
                    if (!dT)
                        return {
                            status: 904,
                            statusText: "attributeType with identifier '" + aT.id + "' must reference a valid dataType"
                        };
        ...
        function checkRelationTypeIds(oTL, rTL) {	// objectTypes, relationTypes
            ...
            for (var i = 0; i < rTL.length; i++) {
                if (!checkEls(oTL, rTL[i].sourceTypes)) return {
                    status: 906,
                    statusText: "sourceTypes of relationType with identifier '" + rTL[i].id + "' must reference valid objectTypes"
                };
                if (!checkEls(oTL, rTL[i].targetTypes)) return {
                    status: 907,
                    statusText: "targetTypes of relationType with identifier '" + rTL[i].id + "' must reference valid objectTypes"
                }
            }
            ...
            function checkEls(L, els) {
                if(!L||!els)return false;
                // each value in els must be the id of a member of L:

                return els.every(function (entry) {
                    return !(indexById(L, entry) < 0);
                });
            }
        }
        ...
        function checkRelationIds(oL, rL) {	// objects, relations
            ...
            for (var i = 0; i < rL.length; i++) {
                if (indexById(oL, rL[i].source.id) < 0) return {
                    status: 908,
                    statusText: "source of relation with identifier '" + rL[i].id + "' must reference a valid object"
                };
                if (indexById(oL, rL[i].target.id) < 0) return {
                    status: 909,
                    statusText: "target of relation with identifier '" + rL[i].id + "' must reference a valid object"
                };
                ...
                if (iL[i].attributes)
                    for (var a = 0, A = iL[i].attributes.length; a < A; a++) {
                        if (iL[i].attributes[a].value) {
                            ...
                            if (!aT) return {
                                status: 920,
                                statusText: "attributes of instance with identifier '" + iL[i].id + "' must reference valid attributeTypes"
                            };
                            ...
                            switch (dT.type) {
                                case 'xs:string':
                                    if (iL[i].attributes[a].value.length > dT.maxLength) return {
                                        status: 921,
                                        statusText: "strings must not exceed maxLength"
                                    };
                                    break;
                                case 'xs:double':
//								if( (iL[i].attributes[a].value*Math.pow(10,dT.accuracy)%1)==0 ) return {status:922;
                                // no break;
                                case 'xs:integer':
                                    if (iL[i].attributes[a].value < dT.min) return {
                                        status: 923,
                                        statusText: "numbers must be larger than min"
                                    };
                                    if (iL[i].attributes[a].value > dT.max) return {
                                        status: 924,
                                        statusText: "numbers must be smaller than max"
                                    };
                                    break;
                                /*							case 'xs:boolean':
                                ...
                                case 'xs:enumeration':
                                    ...
                                    if (vL.length > 1 && !aT.multiple) return {
                                        status: 926,
                                        statusText: "attribute may not have more than one value"
                                    };
                                    for (var v = 0, V = vL.length; v < V; v++) {
                                        vL[v] = vL[v].trim();
                                        if (vL[v] && indexById(dT.values, vL[v]) < 0) return {
                                            status: 927,
                                            statusText: "enumerated valus must be defined by the respective attribute type"
                                        }
                                    }
                            }
                        }
